...
const Pagination = ({ activePage, setActivePage, totalPages }: IPagination) => {
  ...
  const isPageVisible = (page: number) => {
    ...
    if (activePage <= truncationLimit - 1 && page <= truncationLimit) {
      ...
      activePage >= totalPages - 1 &&
      page > totalPages - truncationLimit
    ) {
      return true;
    }
  ...
  const pages = pagesNum.map((page, i, arr) => {
    ...
    const shouldTruncateBefore =
      page !== 1 && !isPrevPageVisible && isPageVisible(page);

    if (shouldTruncateBefore) return <li key={`page-section-${page}`}>...</li>;
    return (
      isPageVisible(page) && (
  ...
  const pagination = (
    <>
      ...
      <li>
        <button
          onClick={() =>
            setActivePage((prev: number) => Math.min(prev + 1, totalPages))
          }
  ...
  return (
    <ol className="flex justify-center gap-2 text-sm font-medium items-center">
      {pagination}
    </ol>
  );
};
